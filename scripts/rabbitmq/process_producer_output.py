...
def unpack_throughput_data(replicas):
...
if __name__ == '__main__':
    ...
    for replicas in range(1, 4):
        for throughput in range(5, 55, 5):
    ...
    producers, consumers, low, high = unpack_throughput_data(2)
    plt.errorbar(producers, consumers, yerr=(low, high),
                 fmt='-^',
                 barsabove=True,
                 color='#58D68D',
                 ecolor='k',
    ...
    producers, consumers, low, high = unpack_throughput_data(3)
    plt.errorbar(producers, consumers, yerr=(low, high),
                 fmt='-s',
                 color='#EB984E',
                 barsabove=True,
    ...
    plt.ylim(bottom=0)
    plt.xlim(left=0)
    plt.title('producers=1, consumers=1, queues=1, record-size=512 bytes')
